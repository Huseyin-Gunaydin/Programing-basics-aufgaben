...
for(let i=0; i<player1.hand.length;i++){
 ...
 else if (player1.hand.length > 0 && player1.hand[i].color !== gameStack.   color || player1.hand[i].number !== gameStack.number){
    getCardOne(player1);
 }
 else if (player1.hand.length > 0 && player1.hand[i].color === gameStack.   color || player1.hand[i].number === gameStack.number){
    gameStack = player1.hand[i];
...
console.log("Aktuelle gamestack", {gameStack});
console.log("-------------");
...
// 3. Spielvorbereitung
// Wir haben viel Code wiederholt. Können wir das eleganter l|| gameStack.numberwhile (i < meyve.length) {
